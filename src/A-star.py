...
def reconstruct_path(came_from: dict[tuple[np.float64, np.float64], tuple[np.float64, np.float64]],
                     start: tuple[np.float64, np.float64], goal: tuple[np.float64, np.float64]) -> list[tuple[np.float64, np.float64]]:
    """
...
def construct_path(data_path:str,nearest_incomp:dict,nearest_outcomp:dict,compo_dict:dict):
    ...
    for route in route_list:
        ...
        for idx in range(len(route)-1):
            ...
            best_start,best_target,shortest_path = find_shortest_path(start_name,targe_name,compo_dict)
...
if __name__ == "__main__":
    ...
    path_list = construct_path("Data\input1.txt",sg.nearest_incomp,sg.nearest_outcomp,sg.compo_dict)
    # print(path_list)

    colors = ['blue', 'black', 'red', 'purple', 'orange']

    for i,each_path in enumerate(path_list):
